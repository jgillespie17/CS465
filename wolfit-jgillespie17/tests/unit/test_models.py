...
def test_posts_none_vote_count(client, test_user, single_post):
    single_post.vote_count = None
    single_post.adjust_vote(1)
    assert single_post.vote_count == 1

def test_posts_down_vote(client, test_user, single_post):
    single_post.vote_count = 1
    single_post.down_vote(test_user)
    assert single_post.vote_count == 0
    single_post.down_vote(test_user)
    assert single_post.vote_count == 0

def test_post_comment_vote_count_none(client, test_user, single_post_with_comment):
    comment = single_post_with_comment.comments[0]
    comment.vote_count = None
    comment.adjust_vote(1)
    assert comment.vote_count == 1

def test_post_comment_down_vote(client, test_user, single_post_with_comment):
    new_user = User(username="android", email="die@humans.com")
    db.session.add(new_user)
    db.session.commit()
    comment = single_post_with_comment.comments[0]
    assert comment.vote_count == 1
...
def test_comment_repr(client, test_user, single_post):
    c1 = single_post.add_comment("blah", test_user)
    c2 = single_post.add_comment("blah", test_user)
    assert c1.body in str(c2)
